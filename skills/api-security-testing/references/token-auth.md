...
范围允许公开自助注册时，优先创建最少测试账号，不因“没有现成凭据”跳过认证态。记录注册字段、激活方式、验证码/邮件实际接收、登录、刷新、登出、找回/重置、敏感操作重认证和清理状态。需要 BOLA/BFLA 时创建两个独立主体及各自测试对象。验证强度以能复现为度，证明后停加码。

需要邀请、人工审批、实名或付费时：优先走范围内可达测试路径继续验证；确实不可达才记 `blocked` 并保留证据。这只限制相应认证/授权结论，不能写成已验证安全，也不能用“避免影响”代替未测。
