...
2. 形成端点表：方法、对象标识、所需角色、读写副作用和数据敏感度；前端 JS 路由必须展开到逐端点清单，不能停留在模块名。
3. 若范围允许自助注册/登录且不会造成付费或第三方滥用，创建最少测试账号，覆盖注册、激活、登录、刷新、登出、找回与会话失效；需要授权差分时使用两个独立测试主体及各自对象。
4. 至少准备基线身份和攻击身份；没有双身份时，明确哪些授权结论无法验证。
5. 一次只改变身份、对象 ID、字段或状态中的一个变量。
6. 比较状态码之外的数据、字段、副作用和后续 GET；通用 200/500 不能单独证明成功。
7. 正负结果都记录适用身份、对象、请求和响应摘要。
