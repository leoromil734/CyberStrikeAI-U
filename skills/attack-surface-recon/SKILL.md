...
输入必须包含目标类型、in-scope 边界、已完成来源和扫描模式。先读 Do-Not-Repeat；上游已有结果时只补缺口。完整矩阵、资产评分与 JS/API 提取读取 `references/comprehensive-recon.md`。
...
4. Web 入口由 `katana`、`gau`、`waybackurls` 互补；下载 HTML 引用、懒加载 chunk、worker 和 source map，逐文件提取 API base、method/path、参数、WebSocket、上传/回调、认证与环境配置，再对目标侧可达性去重验证。
5. `arjun`/`x8` 补参数，`ffuf`/`dirsearch` 补路径；先建立随机不存在路径和 SPA shell 的 hash/title/length 基线，通配 200 不计为命中。
6. `nuclei` 只在存活去重目标上按技术栈裁剪运行，输出全部视为 tentative。
